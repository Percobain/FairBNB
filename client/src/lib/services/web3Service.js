...
class Web3Service {
    ...
    async uploadToIPFS(file) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to upload to IPFS: ${response.statusText}`
                );
            }
    ...
    async uploadMetadata(metadata) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to upload metadata to IPFS: ${response.statusText}`
                );
            }
    ...
    /**
     * Upload file to both IPFS and Greenfield
     */
    async uploadToStorage(file, tokenId, isMetadata = false) {
        const results = {
            ipfs: { success: false },
            greenfield: { success: false },
        };

        // Upload to IPFS first (primary)
        try {
            if (isMetadata) {
                results.ipfs = await this.uploadMetadata(file);
            } else {
                results.ipfs = await this.uploadToIPFS(file);
            }
        } catch (error) {
            console.error("IPFS upload failed:", error);
        }

        // Upload to Greenfield (secondary) - with proper SDK
        try {
            // Get provider for Greenfield
            const provider = await window.ethereum;

            // Initialize Greenfield service with SDK
            const initResult = await greenfieldService.initialize(
                this.account,
                provider
            );
            ...
            if (initResult.success) {
                if (isMetadata) {
                    results.greenfield = await greenfieldService.uploadMetadata(
                        file,
                        tokenId
                    );
                } else {
                    results.greenfield = await greenfieldService.uploadImage(
                        file,
                        tokenId
                    );
                }
            ...
            console.warn("Greenfield upload failed (non-critical):", error);
        }
    ...
    /**
     * Mint a new property NFT with dual storage
     */
    async mintProperty(metadata, imageFile) {
        try {
            ...
            // Generate temporary token ID for file naming
            const tempTokenId = Date.now().toString();

            // Upload image to both IPFS and Greenfield
            const imageResults = await this.uploadToStorage(
                imageFile,
                tempTokenId,
                false
            );

            if (!imageResults.ipfs.success) {
                throw new Error(
                    `Failed to upload image to IPFS: ${imageResults.ipfs.error}`
                );
            }

            // Create metadata with image URI (use IPFS as primary)
            const propertyMetadata = {
                ...metadata,
                image: imageResults.ipfs.url,
                greenfield_image: imageResults.greenfield.success
                    ? imageResults.greenfield.url
                    : null,
                attributes: [
                    {
                    ...
                    {
                        trait_type: "Storage",
                        value: imageResults.greenfield.success
                            ? "IPFS + Greenfield"
                            : "IPFS",
                    },
                ],
            ...
            // Upload metadata to both IPFS and Greenfield
            const metadataResults = await this.uploadToStorage(
                propertyMetadata,
                tempTokenId,
                true
            );

            if (!metadataResults.ipfs.success) {
                throw new Error(
                    `Failed to upload metadata to IPFS: ${metadataResults.ipfs.error}`
                );
            }

            // Mint NFT with metadata URI (use IPFS as primary)
            const tx = await this.contract.mintProperty(
                metadataResults.ipfs.url
            );
            ...
            if (mintEvent) {
                ...
                return {
                    ...
                    metadataUrl: metadataResults.ipfs.url,
                    imageUrl: imageResults.ipfs.url,
                    greenfield: {
                        metadata: metadataResults.greenfield.success
                            ? metadataResults.greenfield.url
                            : null,
                        image: imageResults.greenfield.success
                            ? imageResults.greenfield.url
                            : null,
                    },
                };
            ...
            for (const tokenId of userTokens) {
                try {
                    ...
                    const listingDetails =
                        await this.contract.getListingDetails(tokenId);

                    // Get rental details if exists
                    const rentalDetails = await this.contract.getRentalDetails(
                        tokenId
                    );
        ...
        try {
            ...
            for (let i = 0; i < result.availableTokenIds.length; i++) {
                try {
                    ...
                    console.error(
                        `Failed to get details for listing ${i}:`,
                        error
                    );
                }
            }
    ...
    async rentProperty(tokenId, totalAmount) {
        try {
            ...
            const tx = await this.contract.rentProperty(tokenId, {
                value: totalAmount,
            });
            const receipt = await tx.wait();
    ...
    async getDisputeResolution(tokenId) {
        try {
            ...
            const rentalDetails = await this.contract.getRentalDetails(tokenId);
            const listingDetails = await this.contract.getListingDetails(
                tokenId
            );

            if (!rentalDetails.isDisputed && !rentalDetails.isActive) {
                // Dispute has been resolved
                const juryReward = ethers.formatEther(
                    listingDetails.disputeFee / 2n
                );
                ...
                return {
                    ...
                    tenantWins,
                    juryReward,
                    totalDisputeFee: ethers.formatEther(
                        listingDetails.disputeFee
                    ),
                };
            }
