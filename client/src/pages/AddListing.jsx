...
export function AddListing() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            // Mint NFT with metadata and image (uploads to both IPFS and Greenfield)
            ...
            const storageInfo =
                mintResult.greenfield?.metadata && mintResult.greenfield?.image
                    ? "Stored on IPFS + BNB Greenfield SDK!"
                    : "Stored on IPFS (Greenfield SDK backup attempted)";
                                ...
                                {!selectedImage ? (
                                    <div className="border-2 border-dashed border-nb-ink rounded-nb p-8 text-center">
                                        ...
                                        <p className="text-sm text-nb-ink/70 mb-4">
                                            ...
                                            property (will be stored on IPFS)
                                        </p>
                                        <input
                                            type="file"
                                            accept="image/*"
                                ...
                                {isSubmitting && (
                                    <div className="bg-nb-accent/20 border-2 border-nb-accent rounded-nb p-4">
                                        ...
                                        <p className="text-sm text-nb-ink/70 mt-2">
                                            ...
                                            {uploadProgress >= 20 &&
                                                uploadProgress < 80 &&
                                                "Uploading to IPFS & BNB Greenfield SDK..."}
                                            {uploadProgress >= 80 &&
                                                uploadProgress < 100 &&
                                                "Minting NFT on BSC..."}
                                            {uploadProgress === 100 &&
                                                "Listing property on-chain..."}
                                        </p>
                                    </div>
                                )}
                            </div>
