...
export function ListingDetails() {
    ...
    const generateRealisticAmenities = (propertyType) => {
        ...
        const typeAmenities =
            amenityPools[propertyType] || amenityPools["Apartment"];
        const selectedAmenities = [
            ...typeAmenities.basic,
            ...typeAmenities.comfort.slice(
                0,
                Math.floor(Math.random() * 3) + 1
            ),
            ...typeAmenities.premium.slice(
                0,
    ...
    useEffect(() => {
        const loadProperty = async () => {
            try {
                ...
                const metadataResult = await web3Service.getMetadataFromURI(
                    tokenURI
                );
                ...
                const listingResult =
                    await web3Service.contract.getListingDetails(id);
                const rentalResult =
                    await web3Service.contract.getRentalDetails(id);
                ...
                const isInvolved =
                    rentalResult.isActive &&
                    (rentalResult.tenant.toLowerCase() ===
                        currentAccount.toLowerCase() ||
                        rentalResult.landlord.toLowerCase() ===
                            currentAccount.toLowerCase());

                setIsCurrentUserInvolved(isInvolved);

                // Generate realistic mock data
                const realisticPhotos = generateRealisticPhotos(
                    metadata.propertyType,
                    metadata.city
                );
                realisticPhotos[0] = imageUrl; // Keep the original image as first

                const realisticAmenities = generateRealisticAmenities(
                    metadata.propertyType
                );
                const realisticReviews = generateRealisticReviews(
                    metadata.propertyType,
                    metadata.city
                );
                ...
                const propertyData = {
                    ...
                    avgRating: (
                        realisticReviews.reduce(
                            (acc, review) => acc + review.rating,
                            0
                        ) / realisticReviews.length
                    ).toFixed(1),
    ...
    const handleBooking = async (bookingData) => {
        try {
            ...
            // Rent the property
            const rentResult = await web3Service.rentProperty(
                property.tokenId,
                totalAmount
            );
            ...
            toast.success("Booking confirmed!", {
                description: `Property rented successfully. Transaction: ${rentResult.txnHash.slice(
                    0,
                    10
                )}...`,
            });
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <Gallery
                            ...
                            coverIndex={property.coverImage}
                        />
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-nb-ink/70">
                                            This property is under an active
                                            rental agreement with escrow
                                            protection.
                                        </p>
                                        {isCurrentUserInvolved && (
                                            <p className="text-sm text-nb-accent font-medium mt-1">
                                                You are involved in this rental
                                                agreement.
                                            </p>
                                        )}
                                    </div>
                        ...
                        <NBCard>
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    <h3 className="font-display font-bold text-lg text-nb-ink">
                                        Hosted by{" "}
                                        {property.landlord.slice(0, 6)}...
                                        {property.landlord.slice(-4)}
                                    </h3>
                                    <div className="flex items-center text-sm text-nb-ink/70">
                                        <Shield className="w-3 h-3 mr-1" />
                                        <span>
                                            Verified host • Token ID:{" "}
                                            {property.tokenId}
                                        </span>
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
                                            ...
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-sm text-nb-ink">
                                                    {property.minDurationMonths}
                                                    -
                                                    {property.maxDurationMonths}{" "}
                                                    months
                                                </span>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                            ...
                                            {property.amenities.map(
                                                (amenity) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={amenity}
                                                            className="flex items-center space-x-3 p-3 border border-nb-ink/20 rounded-nb"
                                                        >
                                                            <IconComponent className="w-5 h-5 text-nb-accent" />
                                                            <span className="text-nb-ink font-medium">
                                                                {amenity}
                                                            </span>
                                                        </div>
                                                    );
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex justify-between items-center py-3 border-b border-nb-ink/20">
                                                ...
                                                <span className="font-bold text-nb-ink">
                                                    ₹
                                                    {property.disputeFee.toLocaleString()}
                                                </span>
                                            </div>
                                            <div className="flex justify-between items-center py-3 border-b border-nb-ink/20">
                                                <span className="text-nb-ink font-medium">
                                            ...
                                            <div className="flex justify-between items-center py-3 bg-nb-accent/10 px-4 rounded-nb">
                                                <span className="text-nb-ink font-bold">
                                                    Total Initial Payment
                                                </span>
                                                <span className="font-bold text-nb-ink text-lg">
                                                    ₹
                                                    {(
                                                        property.rentPerMonth +
                                                        property.securityDeposit +
                                            ...
                                            <div className="mt-6 p-4 bg-nb-bg border border-nb-ink/20 rounded-nb">
                                                ...
                                                <ul className="text-sm text-nb-ink/70 space-y-1">
                                                    <li>
                                                        • Security deposit is
                                                        fully refundable at the
                                                        end of the lease
                                                    </li>
                                                    <li>
                                                        • Dispute fee is held in
                                        ...
                                        <div className="space-y-6">
                                            {property.reviews.map(
                                                (review, index) => (
                                                    <div
                                                        key={index}
                                                        className="border-b border-nb-ink/20 pb-6 last:border-b-0"
                                                    >
